...
/// <summary>Input lock is runtime policy. It is not persisted and is re-applied on every new connection.</summary>
public sealed class GuardedTerminal : TerminalControl
{
    ...
    {
        AddHandler(KeyDownEvent, (_, e) => { if (InputLocked && !(e.Key == Key.C && e.KeyModifiers.HasFlag(KeyModifiers.Control))) e.Handled = true; }, RoutingStrategies.Tunnel);
        AddHandler(TextInputEvent, (_, e) => { if(InputLocked) e.Handled = true; }, RoutingStrategies.Tunnel);
    }
}

/// <summary>Owns one terminal, one session lifecycle, one capture runtime. Reparenting never calls Start/Stop.</summary>
public sealed class SessionRuntime : IDisposable
{
    ...
    public bool Locked { get => Terminal.InputLocked; set { Terminal.InputLocked = value; Terminal.IsHitTestVisible = !value; Changed?.Invoke(); } }
    ...
    public SessionRuntime(Guid id, TerminalSessionProfile profile, ProfileRepository profiles, bool design)
    {
        ...
        Terminal = new GuardedTerminal(profiles) { Columns = 120, Rows = 36, Padding = new Thickness(14, 8, 8, 4), VtProcessorPreference = design ? VtProcessorPreference.Managed : VtProcessorPreference.Auto, PreserveScrollbackOnSessionStart = true, SixelGraphicsEnabled = true };
        ...
        Terminal.SelectionFinalized += async (_, _) => { if(Profile.Behavior.CopyOnSelectEnabled) { try { await Terminal.CopySelectionAsync(); } catch(Exception ex) { Error = ex.Message; } } };
        Capture = new TerminalCaptureRuntime(Terminal); Capture.StateChanged += (_, _) => Changed?.Invoke();
        Locked = IsProduction; ApplyProfile(profile);
    }
    public void ApplyProfile(TerminalSessionProfile profile)
    {
        Profile = profile; var a = profile.Appearance;
        Terminal.FontFamilyName = a.FontFamilyName; Terminal.FontSource = a.FontSource; Terminal.FontFilePath = a.FontFilePath ?? ""; Terminal.TerminalFontSize = a.FontSize;
        Terminal.AutoScroll = a.AutoScroll; Terminal.ScrollbackLimit = profile.Layout.ScrollbackLimit; Terminal.ReflowOnResize = profile.Behavior.ReflowOnResize; Terminal.SixelGraphicsEnabled = profile.Behavior.SixelGraphicsEnabled;
        ...
        Terminal.TextHighlightRules = a.TextHighlightRules.Where(r => r.IsEnabled).Select(r => new TerminalTextHighlightRule { Name = r.Name, Pattern = r.Pattern, Foreground = Parse(r.ForegroundColor), Background = Parse(r.BackgroundColor), DarkForeground = Parse(r.DarkForegroundColor), DarkBackground = Parse(r.DarkBackgroundColor) }).ToArray();
        var font = a.FontRendering; Terminal.FontSubpixelPositioning = font.SubpixelPositioning; Terminal.FontEdging = font.Edging; Terminal.FontHinting = font.Hinting; Terminal.FontBaselineSnap = font.BaselineSnap; Terminal.FontEmbeddedBitmaps = font.EmbeddedBitmaps; Terminal.FontEmbolden = font.Embolden; Terminal.FontForceAutoHinting = font.ForceAutoHinting; Terminal.FontLinearMetrics = font.LinearMetrics;
        ThemeManager.ApplyTerminal(Terminal);
    }
    public Task EnsureStartedAsync() => _startup ??= StartAsync();
    public async Task StartAsync()
    {
        await _lifecycle.WaitAsync(_lifetime.Token);
        try
        {
            if(_disposed || IsRunning || IsReplay) return;
        ...
        catch(Exception ex) { Error = ex.Message; State = "Connection failed"; Terminal.WriteOutput(Encoding.UTF8.GetBytes("\r\nTessera: " + Safety.CleanTitle(ex.Message) + "\r\nUse Session → Reconnect after updating this profile.\r\n")); Changed?.Invoke(); }
        finally { _lifecycle.Release(); }
    }
    public async Task ReconnectAsync()
    {
        if(_disposed || IsReplay) return;
        Terminal.StopPty(); await StartAsync();
    }
...
public sealed class SessionRegistry(ProfileRepository profiles, bool design) : IDisposable
{
    ...
    public SessionRuntime Get(TerminalDocument document)
    {
        ...
        var session = new SessionRuntime(document.Id, profiles.Get(document.ProfileId), profiles, design); _sessions.Add(document.Id, session); return session;
    }
