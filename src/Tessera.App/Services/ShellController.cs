...
public sealed class ShellController : IDisposable
{
    ...
    public SessionRuntime GetSession(TerminalDocument document)
    {
        ...
        runtime.Terminal.ShellIntegrationEventReceived += (_, e) =>
        {
            ...
            if(completed is not null) Dispatcher.UIThread.Post(() => { CommandHistory.Insert(0, completed); if(CommandHistory.Count > 5000) CommandHistory.RemoveRange(5000, CommandHistory.Count - 5000); Changed?.Invoke(false); });
        };
    ...
    public void Apply(Workspace workspace, bool undoable = true, bool render = true)
    {
        ...
        if(ActiveDocumentId is not {} active || !workspace.Documents.ContainsKey(active)) ActiveDocumentId = Layout.Groups(workspace.Root).First().Active;
        Notify(render);
    }
    ...
    public void Move(Guid documentId, Guid target, DockEdge edge) { Apply(Layout.Move(Active, documentId, target, edge)); ActiveDocumentId = documentId; Notify(true); }
    ...
    public void SwitchWorkspace(Guid workspaceId)
    {
        ...
        Data = Data with { ActiveWorkspace = workspaceId }; _history.Clear(); FocusMode = false; ActiveDocumentId = Layout.Groups(Active.Root).First().Active; Notify(true);
    }
    ...
    public void SaveLayout(string name) { Data = Data with { Layouts = [.. Data.Layouts, new(Guid.NewGuid(), name.Trim(), Active.Root, new(Active.Documents))] }; Notify(true); }
    ...
    public async Task FlushAsync() { if(!DesignMode && Initialized && _storageHealthy) await _store.SaveAsync(Data); }
    public void Dispose() { if(_disposed) return; _disposed = true; _saveTimer.Stop(); BroadcastEnabled = false; Sessions.Dispose(); }
}
